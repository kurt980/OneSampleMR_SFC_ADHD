...
# Mendelian Randomization Analysis (Main Analysis Reverse MR)

# 3. Instrumental Variable Selection: Clumping and Thresholding (C+T)
...
unrels <- c("", "_unrelated") # whether done on all obs or unrelated obs
# harms <- c("_sd_harm2") # harm2 and fisher+harm2
methods <- c("dsm5_gwasplink19", "dsm5_gwasplink20") # c("gwasplink20_covsex", "dsm5_fastGWA_covsex")
dsm5s <- c("adhd") #c("depress", "anxdisord", "somaticpr", "adhd", "opposit", "conduct")
