...
02/15: This is changed to doing GWAS for sensitivity analysis (RUNNING ON MAC PRO)
  - GWAS on unharmonized SFC, on unrelated samples
  - GWAS on harmonized SFC, on unrelated samples, but without sex.
...
directory <- "/Users/grc8mu/Desktop/DS/20250215_GWAS/"
rois <- c(1:379)
...
**Phenotype not harmonized created using Harmoization.R in utilities**


```{r}
QCs <- c("_qc") # qc and no qc
unrels <- c("_unrelated") # all obs and unrelated obs c("")
...
for (qc in QCs) {
  for (unrel in unrels) {
    ...
    ########1. GWAS with PLINK2.0, on unharmonized, with sex (no age) covariate (--glm)#########
    method <- "gwasplink20_covsex"
    #+++++++++Get covariates++++++++++++++++++++++++++++++++++++++++++++++++
    covar_sex <- paste0(cov_path, "covar_sex.txt")
    #+++++++++Set work directory++++++++++++++++++++++++++++++++++++++++++++++
    work_dir <- paste0(directory, method, qc, unrel, "_unharm/")
    ...
    foreach(roi = rois, .combine = list) %dopar% {
      tryCatch({
        #+++++++++get SCFC roi phenotypes+++++++++++++++++++++++++++++++++++++++
        roi_pheno <- paste0(pheno_path, "scfc_sd_fam_roi", roi, ".phen")
        #+++++++++Name output file name+++++++++++++++++++++++++++++++++++++++++
        outfile <- paste0(work_dir, method, qc, unrel, "_scfc_sd_fam_roi", roi)
        #+++++++++Run GWAS commands+++++++++++++++++++++++++++++++++++++++++++++
        command <- paste0(tool_path, "plink2 --bfile ", bfile, " ",
                          ...
                          "--out ", outfile)
        system(command)
        cat(method, qc, unrel, " executed successfully for:", "roi_", roi, "\n")
    ...
    # close(readme_conn)
    # close(count_conn)
    cat(paste0("***************Finished running commands:", method, qc, unrel, "_unharm***************\n\n\n"))
    #=========================================================================
    
    ########2. GWAS with PLINK2.0, on harmonized, with (no sex) (no age) covariate (--glm)#########
    method <- "gwasplink20_nocov"
    #+++++++++Get covariates++++++++++++++++++++++++++++++++++++++++++++++++
    # covar_sex <- paste0(cov_path, "covar_sex.txt")
    #+++++++++Set work directory++++++++++++++++++++++++++++++++++++++++++++++
    work_dir <- paste0(directory, method, qc, unrel, "_harm2/")
    if (!dir.exists(work_dir)) { dir.create(work_dir, recursive = F) }
    #+++++++++Create readme+++++++++++++++++++++++++++++++++++++++++++++++++++
    readme_path <- paste0(work_dir, "readme.txt")
    readme_conn <- file(readme_path, "w")
    ...
    foreach(roi = rois, .combine = list) %dopar% {
      tryCatch({
        #+++++++++get SCFC roi phenotypes+++++++++++++++++++++++++++++++++++++++
        roi_pheno <- paste0(pheno_path, "scfc_sd_harm2_fam_roi", roi, ".phen")
        #+++++++++Name output file name+++++++++++++++++++++++++++++++++++++++++
        outfile <- paste0(work_dir, method, qc, unrel, "_scfc_sd_harm2_fam_roi", roi)
        #+++++++++Run GWAS commands+++++++++++++++++++++++++++++++++++++++++++++
        command <- paste0(tool_path, "plink2 --bfile ", bfile, " ",
                          "--pheno ", roi_pheno, " ",
    ...
    # ##########?. GWAS with fastGWA, with sex covariate########################
